...
test('setMedia works for setting media below the max length', () => {
	for (let _ = 0; _ < 100; _++) {
		...
		expect(game.actors, 'Actors are removed after the media index and retained before').toEqual(
			expectedActors
		);
		expect(game.media, 'Media are removed after the media index and retained before').toEqual(
			expectedMedia
		);
	}
});
...
test('setActor works for setting an actor in the middle', () => {
	for (let _ = 0; _ < 100; _++) {
		...
		expect(game.actors, 'After setting an actor, all following actors should be cleared').toEqual(
			expectedActors
		);
		expect(
			game.media,
			`[${index}] After setting an actor, all following media should be cleared`
...
test('setActor works for setting an actor at the end', () => {
	for (let _ = 0; _ < 100; _++) {
		...
		expect(game.media, 'After setting an actor, all following media should be cleared').toEqual(
			expectedMedia
		);
	}
});
...
test('gameHasWon can only return true if both actors are in the list', () => {
	for (let _ = 0; _ < 100; _++) {
		...
		expect(
			game.gameHasWon(),
			'If a game does not have both actors, it should return false'
		).toEqual(false);
		game.swap();
		expect(
			game.gameHasWon(),
...
test('gameHasWon must return true if both actors are in the list', () => {
	for (let _ = 0; _ < 100; _++) {
		...
		expect(
			game.gameHasWon(),
			`[${_}] game with actor1 and actor2 at the end doesn't detect that the game has been won`
		).toEqual(true);
		...
		expect(
			game.gameHasWon(),
			`[${_}] game with actor2 and actor1 at the end doesn't detect that the game has been won`
		).toEqual(true);
		...
		expect(
			game.gameHasWon(),
			`[${_}] game with actor1 in the middle doesn't detect that the game has been won`
		).toEqual(true);
		...
		expect(
			game.gameHasWon(),
			`[${_}] game with actor2 in the middle doesn't detect that the game has been won`
		).toEqual(true);
		...
		expect(
			game.gameHasWon(),
			`[${_}] game with both actors in the middle doesn't detect that the game has been won`
		).toEqual(true);
	}
});
