...
{#if $game}
	<div class="w-3/4 h-3/4 bg-surface-800">
		<h2 class="h2 text-center pt-4 text-primary-300">
			{$game.actors[0]?.name} to {$game.getLastActor()?.name}
		</h2>
		...
		<h2 class="h2 text-center">
			{#if totalTime}
				Completed in {totalTime.getMinutes()}:{totalTime.getSeconds() < 10
					? '0'
		...
		<div class="flex space-evenly w-full justify-center mt-8">
			<button
				class="btn variant-filled-surface mx-10"
				on:click={() => {
					window.location.reload();
				}}>Try this pairing again</button
			>
			<a class="cursor-pointer btn variant-filled-surface mx-10" href="/play" data-sveltekit-reload
				>Try a new game</a
			>
		</div>
	</div>
